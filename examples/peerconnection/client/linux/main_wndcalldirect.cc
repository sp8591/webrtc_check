...
bool GtkMainWnd::Create() {
  ...
  if (window_control) {
    gtk_window_set_position(GTK_WINDOW(window_control), GTK_WIN_POS_CENTER);
    gtk_window_set_default_size(GTK_WINDOW(window_control), 640, 480);
    gtk_window_set_title(GTK_WINDOW(window_control), "calldirect");
  ...
  if (window_) {
    ...
    gtk_window_set_title(GTK_WINDOW(window_), "video");
    // g_signal_connect(G_OBJECT(window_), "delete-event",
    //                  G_CALLBACK(&OnDestroyedCallback), this);
    // g_signal_connect(window_, "key-press-event", G_CALLBACK(OnKeyPressCallback),
    //                  this);
...
void GtkMainWnd::SwitchToConnectUI() {
  ...
  gtk_container_set_border_width(GTK_CONTAINER(window_control), 10);
  ...
  gtk_container_add(GTK_CONTAINER(window_control), vbox_);
...
void GtkMainWnd::StartLocalRenderer(webrtc::VideoTrackInterface* local_video) {
  ...
  remote_renderer_.reset(new VideoRenderer(this, local_video));
}
